...
app.post('/api/save-project', authenticateToken, async (req, res) => {
    try {
        ...
        const userEmail = req.user.email; // Get email from authenticated user token

        // Modified query to include user_email
        const query = `
            INSERT INTO projects 
            (project_name, region, department, tyre_size, protocol, status, created_at, user_email)
...
// Update the project history endpoint to require authentication
